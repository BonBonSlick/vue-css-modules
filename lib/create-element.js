...
function createElement(_) {
  ...
  if ((0, _utils.isObject)(data)) {
    ...
    if(injectAttr === 'class') {
      ...
      data[injectAttr] = data.staticClass.concat(` ${expression && typeof expression === 'string' ? expression : ''}`);
    }
        ...
        if (module && typeof module === 'string') {
          ...
          for (var _i in classExpressions) {
            ...
            if(['staticClass', 'class'].indexOf(injectAttr) >= 0) {
              if(data.staticClass.indexOf(className) >= 0) {
                data.staticClass = data.staticClass.replace(className, hit);
              }else{
